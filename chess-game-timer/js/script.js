...
const timesUp = new Audio('audio/BabyElephantWalk60.wav');
const click = new Audio('audio/StarWars60.wav');
...
// Warn the player if time drops to 30 seconds 
const timeWarning = (player, min, sec) => {
    // change the numbers to red below 0 mintues and 30 seconds
    if(min < 1 && sec <= 30) {
        if(player === 1) {
...
// Swap player's time after a move (player1 = 1, player2 = 2)
const swapPlayer = () => {
        if(!playing) return;
    ...
    let timerId = setInterval(function() {
        ...
        if(currentPlayer === 1) {
            if(playing) {
                ...
                p1time.minutes = parseInt(p1time.getMintues('min1'), 10);
                if(p1sec === 60) {
...
timerPanel_Start.addEventListener('click', swapPlayer);
timerPanel_Reset.addEventListener('click', swapPlayer);

// Loop through the start and reset buttons.

for (let i = 0; i < buttons.length; i++) {
    buttons[i].addEventListener('click', () => {
        if (buttons[i].textContent === 'START' || buttons[i].textContent === 'RESET') {
